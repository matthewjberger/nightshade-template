...
//! ## Architecture
//!
//! - `src/state.rs` — the `TemplatePlugin`. Registers the app member
//!   world, the app resources, and the startup and update systems
//!   against the `App` builder.
...
//! `state.rs::TemplatePlugin::build`.

mod ecs;
mod state;
mod systems;

pub use state::TemplatePlugin;
